...
Local cTitulo := 'Relatorio Base Indicadores' // Titulo da janela de escolhe de
parametros de empresas.
Local aEmp // Recebe empresas e filiais.
Local cArqTmpADM // Nome da tabela temporia na system de Admissùes.
Local cTmpADM := 'TMPADM' // Nome da tabela temporaria de Admissùes.
Local cArqTmpDEM // Nome da tabela temporaria na system deDemissıes(Desligamentos).
Local cTmpDEM := 'TMPDEM' // Nome da tabela temporaria de Demissùes(Desligamentos).
...
Local oExcel := FWMsExcelEx():New() // Instancia objeto que monta excel para
impres„o do relatorio.

CriaPerg(cPerg) // Cria grupo de perguntas.
If Empty(Pergunte(cPerg,.T.)) // Chama o grupo de perguntas.
      MsgAlert('Relatorio Cancelado!')
      Return
Else
      aMeses := GrvMeses() // Grava todos os meses selecionados entre os periodos.
EndIf

cEmp := U_TelaEmp(aEmpresas, cTitulo) // Chama tela para seleùùo de empresas e
filiais.
aEmp := U_EmpToArray(cEmp) // Destrincha o resultado da tela em um vetor com os
codigos das empresas e filiais.

/*Static Functions que cria as tabelas temporarias para receber
os dados das WorkSheets(Planilhas no Excel) no excel*/
cArqTmpADM := CriaTmpADM(cTmpADM) // Cria Tabela temporaria para area de Admissùes.
cArqTmpDEM := CriaTmpDEM(cTmpDEM) // Cria Tabela temporaria para area de
Desligamentos(Demissıes).
cArqTmpBMS := CriaTmpBMS(cTmpBMS) // Cria Tabela temporaria para area de BaseMensal.
...
GrvTmpADM(aEmp, cTrbQry, cTmpADM, aMeses) // Carrega os dados        do Banco e grava na tabela temporaria da area de Admissùes.
GrvTmpDEM(aEmp, cTrbQry, cTmpDEM, aMeses) // Carrega os dados        do Banco e grava na tabela temporaria de Demissùes.
GrvTmpBMS(aEmp, cTrbQry, cTmpBMS, aMeses) // Carrega os dados        do Banco e grava na
tabela temporaria de Base Mensal.

/*Static Functions que criam,        configuram, gravam dados nas WorkSheets(Planilhas no
Excel) */
ConfWSAdm(oExcel, cTmpADM) //        Configura Worksheet(Planilha) de Admissùes.
ConfWSDem(oExcel, cTmpDEM) //        Configura Worksheet(Planilha) de Demissùes.
ConfWSBms(oExcel, cTmpBMS) //        Configura Worksheet(Planilha) de Base Mensal.

oExcel:Activate() // Ativa o Excel.
oExcel:GetXMLFile(Alltrim(mv_par05)+'Base Indicadores(explicativa).xml') //
Exportar o Excel para o diretorio especificado.
...
/*Static Function que cria tabela temporaria para a area de Admissùes*/
Static Function CriaTmpADM(cTmpADM)

Local aEstrutura := {} // Array com os campos das tabela
Local cArqTmpADM // Nome da tabela

/*Monta estrutura para area de Admissùes */
aadd(aEstrutura, {"NMES"     , "C"                    , 2                      , 0}) // Numero do mùs.
aadd(aEstrutura, {"CODEMP"   , "C"                    , 2                      , 0}) // Codigo da empresa.
aadd(aEstrutura, {"CODFIL"   , "C"                    , 2                      , 0}) // Codigo da filial.
aadd(aEstrutura, {"DESCMES"  , "C"                    , 10                     , 0}) // Nome do mÍs.
aadd(aEstrutura, {"DESCFILI" , "C"                    , 41                     , 0}) //Descriùo da filial da empresa do funcionario.
...
aadd(aEstrutura, {"DESCCC"   , TamSX3("CTT_DESC01")[3], TamSX3("CTT_DESC01")[1], TamSX3("CTT_DESC01")[2]}) // Descriùùo do Centro de Custo do Funcionario.
aadd(aEstrutura, {"NOME"     , TamSX3("RA_NOME")[3]   , TamSX3("RA_NOME")[1]   , TamSX3("RA_NOME")[2]}) // Nome do Funcionario.
aadd(aEstrutura, {"FUNCAO"   , TamSX3("RJ_DESC")[3]   , TamSX3("RJ_DESC")[1]   , TamSX3("RJ_DESC")[2]}) // Descriùùo da funcùo do funcionario.
aadd(aEstrutura, {"DTADMISAO", TamSX3("RA_ADMISSA")[3], TamSX3("RA_ADMISSA")[1], TamSX3("RA_ADMISSA")[2]}) // Data de Admissùo do funcionario.
aadd(aEstrutura, {"DTDEMISAO", TamSX3("RA_DEMISSA")[3], TamSX3("RA_DEMISSA")[1], TamSX3("RA_DEMISSA")[2]}) // Data de Demissùo do funcionario.
...
/*Static Function que cria tabela temporaria para a area de
Desligamentos(Demissùes)*/
...
AADD(aEstrutura,{"DESCFILI"         ,"C"                                ,41
                              ,0                                        }) //
Descriùo da filial da empresa do funcionario.
...
AADD(aEstrutura,{"DESCCC"           ,TamSX3("CTT_DESC01")[3]
      ,TamSX3("CTT_DESC01")[1]      ,TamSX3("CTT_DESC01")[2]      }) // Descriùùo do
...
AADD(aEstrutura,{"FUNCAO"           ,TamSX3("RJ_DESC")[3]   ,TamSX3("RJ_DESC")[1]
      ,TamSX3("RJ_DESC")[2]   }) // Descriùùo da funcùo do funcionario.
AADD(aEstrutura,{"DTADMISAO" ,TamSX3("RA_ADMISSA")[3]       ,TamSX3("RA_ADMISSA")[1]
      ,TamSX3("RA_ADMISSA")[2]      }) // Data de Admissùo do funcionario.
AADD(aEstrutura,{"DTDEMISAO" ,TamSX3("RA_DEMISSA")[3]       ,TamSX3("RA_DEMISSA")[1]
      ,TamSX3("RA_DEMISSA")[2]      }) // Data de Demissùo do funcionario.
...
AADD(aEstrutura,{"DTDEMISAO" ,TamSX3("RA_DEMISSA")[3]       ,TamSX3("RA_DEMISSA")[1]
      ,TamSX3("RA_DEMISSA")[2]      }) // Data de Demissùo do funcionario.
AADD(aEstrutura,{"SITFOLHA"         ,TamSX3("RA_SITFOLH")[3]
      ,TamSX3("RA_SITFOLH")[1]      ,TamSX3("RA_SITFOLH")[2]      }) // Situaùùo da
folha do funcionario(' '->SITUACAO NORMAL, 'A'->AFASTADO TEMP., 'D'->DEMITIDO, 'F'-
>FERIAS, 'T'->TRANSFERIDO).
AADD(aEstrutura,{"SALARIO"          ,TamSX3("RA_SALARIO")[3]
      ,TamSX3("RA_SALARIO")[1]      ,TamSX3("RA_SALARIO")[2]      }) // Salario do
Funcionario.
AADD(aEstrutura,{"DIASUTEIS" ,"N"                                   ,2
                        ,0                                          }) // Dias Uteis.
AADD(aEstrutura,{"CH"              ,"C"                                   ,6
                              ,0                                          }) // CH ->
carga horaria.
AADD(aEstrutura,{"CHMESPREV" ,"C"                                   ,6
                        ,0                                          }) // CH Mensal
prevista (Dias Uteis . CH)
AADD(aEstrutura,{"ABONOAUT"        ,"C"                                     ,6
                              ,0                                            }) // Abono
autorizado.
AADD(aEstrutura,{"ATESMED"         ,"C"                                     ,6
                              ,0                                            }) //
Atestado medico.
AADD(aEstrutura,{"CHNAOREA"        ,"C"                                     ,6
                              ,0                                            }) // CH Nùo
realizada, soma(Atestado - Horas negativas trimestre - abono)
AADD(aEstrutura,{"ABSTEISMO" ,"C"                                   ,5
                        ,0                                          }) // Absenteismo,
Percentual(CH Mensal prevista / CH nùo realizada)
AADD(aEstrutura,{"HEPAGA"          ,"C"                                     ,6
                              ,0                                            }) // nùo
sei
AADD(aEstrutura,{"FADESC"          ,"C"                                     ,6
                              ,0                                            }) //
Faltas, atrasos descontados.
AADD(aEstrutura,{"HPOSTRIMES" ,"C"                                  ,6
                        ,0                                          }) // Hora +
trimestre. -> nùo sei
AADD(aEstrutura,{"HNEGTRIMES" ,"C"                                  ,6
                        ,0                                          }) // Hora -
trimestre. -> nùo sei
...
/*Static Function que carrega ù grava os dados
da area de admissùo na tabela temporaria.*/
...
(SRA.RA_CC = CTT.CTT_CUSTO AND CTT.D_E_L_E_T_ = '') "
            cQuery += " INNER JOIN "+U_RetSqlEmp(aEmp[nCont][1],"SRJ")+" SRJ ON
(SRA.RA_CODFUNC = SRJ.RJ_FUNCAO AND SRJ.D_E_L_E_T_ = '') "
            cQuery += " WHERE SRA.RA_FILIAL IN "+cFiliais
            cQuery += "   AND SRA.RA_ADMISSA BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
            cQuery += "   AND SRA.D_E_L_E_T_ = '' "

           // Se jù existir a tabela da query entao fecha
           If Select(cTrbQry) > 0
                 (cTrbQry)->(dbCloseArea())
           EndIf
...
/*Static Function que carrega ù grava os dados
da area de Demissùes na tabela temporaria.*/
...
(SRA.RA_CODFUNC = SRJ.RJ_FUNCAO AND SRJ.D_E_L_E_T_ = '') "
            cQuery += " WHERE SRA.RA_FILIAL IN "+cFiliais
            cQuery += "   AND SRA.RA_DEMISSA BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
            cQuery += "   AND SRA.D_E_L_E_T_ = '' "
...
Local dDemissao // Data de demissùo do funcionario.
Local aAfast := {} // Periodo de afastamento, para calculo de dias uteis.
Local aDiasUteis := {} // Quantidade de dias uteis, e quais sùo os dias uteis.
Local nCH // Recebe a carga horaria diaria do funcionario atualmente posicionado.
Local nCHMesPrev // Recebe a carga horaria mensal prevista pro funcionario
posicionado.
...
Local nPosNegTri // Hora mais menos trimestre -> pega da regra do relatorio TOTVS -
> PONTO ELETRONICO -> RELATORIOS -> BANCO DE HORAS -> RELATORIO DE
HORAS(PONR100.PRX)
Local aTransTudo := {} // Recebe as variaveis transformadas em caracteres, para
impresùo do excel em horas.
...
"
            cQuery += "   AND   (SRA.RA_DEMISSA >= '"+dTos(aMeses[x][4])+"' OR
SRA.RA_DEMISSA = '') "
...
           dbUseArea(.T., 'TOPCONN', TCGenQry(,,cQuery), cTrbQry, .F., .T.)
            ...
            While !(cTrbQry)->(Eof())
                  ...
                  dDemissao := STOD((cTrbQry)->DTDEMISAO) // Data de demissùo do
Funcionario, para calculo dos dias uteis.

                  aFerRCG := VeriFerRCG(cFil, cMat, aEmp, nCont, x, aMeses, cTurno)
/*Verifica os dias de feriado na tabela RCG por mùs*/

                  aAfast := PerAfast(aEmp, nCont, cFil, cMat, aMeses, x) /*Periodo
de Afastamento do funcionario, para calculo de dias uteis.*/
                  aDiasUteis := CalcDUT(cFil, cMat, aMeses, aFerRCG, aAfast,
dDemissao) /*Calcula a quantidade de dias uteis no mùs do funcionario.*/
                  nCH := CalcCH(aEmp, nCont, cTurno) /*Calcula a carga horaria do
funcionario.*/
                  nCHMesPrev := CalcCHMes(aDiasUteis[1][1], nCH) /*Calcula a carga
horaria mensal do funcionario.*/
                  nSomaAbono := SomaAbono(aEmp, nCont, cFil, cMat, aMeses, x)
/*Total de horas abonadas desse funcionario nesse mes.*/
                  nSomaAtest := SomaAtest(aEmp, nCont, cFil, cMat, aMeses, x,
aDiasUteis[1][2], nCH) /*Soma os atestados medicos.*/
                  nCHNaoReal := CHNaoReal(nSomaAbono, nSomaAtest) /*Horas nùo
realizadas, Abonos + Atetados medicos*/
                  nAbstsm := Abstsm(nCHMesPrev, nCHNaoReal) // Percentual do
Absenteismo.
                  nHEPaga := HEPaga(aEmp, nCont, cFil, cMat, aMeses, x) // Hora
extra paga ao funcionario.
                  nFTDesc := HFTDesc(aEmp, nCont, cFil, cMat, aMeses, x) // Hora
Falta e desconto.
                  nPosNegTri := PosNegTri(cFil, cMat, aMeses, x) // Hora mais
trimestre -> pega da regra do relatorio TOTVS -> PONTO ELETRONICO -> RELATORIOS ->
BANCO DE HORAS -> RELATORIO DE HORAS(PONR100.PRX)

                  aTransTudo := TransTudo(nCH, nCHMesPrev, nSomaAbono, nSomaAtest,
nCHNaoReal, nAbstsm, nHEPaga, nFTDesc, nPosNegTri) // Transforma todas as horas pra
impressùo do excel.
...
dados na WorkSheet(Planilha) de Admissùes*/
Static Function ConfWSAdm(oExcel, cTmpADM)

Local cWorkSheet := 'Admissùes' // Recebe o Nome da WorkSheet(Planilha).
Local cTable := 'ADMISSùES - TAB' // Recebe o Nome da tabela.
Local aLinhaADM := {} // Recebe os dados da tabela temporaria de Admissùo linha a
linha.

oExcel:AddWorkSheet(cWorkSheet) // Adiciona uma WorkSheet(Planilha) no   Excel.
oExcel:AddTable(cWorkSheet, cTable) // Adiciona uma tabela dentro da
WorkSheet(Planilha) selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Mes'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Empresa'                           ,1,1) //
Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Matricula'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'CPF'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'CC'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Desc. Centro de custo'     ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Nome'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Funùùo'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Admissùo'                          ,1,4) //
Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Demissùo'                          ,1,4) //
Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Salùrio'                           ,1,2) //
Adiciona uma coluna na tabela selecionada.
(cTmpADM)->(dbGoTop())
While !(cTmpADM)->(EOF())
...
/*Static Function que cria, configura, grava
dados na WorkSheet(Planilha) de Demissùes*/
...
Local aLinhaDEM := {} // Recebe os dados da tabela temporaria de Demissùes linha a
linha.
...
Local aLinhaBMS := {} // Recebe os dados da tabela temporaria de Demissùes linha a
linha.
...
oExcel:AddColumn(cWorkSheet, cTable, 'Mes'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Empresa'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Matricula'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'CPF'
...
oExcel:AddColumn(cWorkSheet, cTable, 'CC'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Desc. Centro de custo'           ,1,1) //
Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Nome'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Funùùo'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Admissùo'
      ,1,4) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Demissùo'
      ,1,4) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Salùrio'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Dias uteis'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'C.H'
      ,1,1) // Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'C.H Mensal prevista'               ,1,1) //
Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Abono autorizado'          ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Atestado medico'           ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'C.H. nùo realizada'        ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Absenteismo'               ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'HE Paga'                   ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'Faltas/Atrasos descontados'        ,1,1) //
Adiciona uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'H ( + ) Trimestre'         ,1,1)   // Adiciona
uma coluna na tabela selecionada.
oExcel:AddColumn(cWorkSheet, cTable, 'H ( - ) Trimestre'         ,1,1)   // Adiciona
uma coluna na tabela selecionada.
...
/*------------------------------------Static Function
Auxiliares------------------------------------*/
/*Verifica os dias de feriados, final de semana, dias nùo trabalhados na
tabela RCG por periodo e turno, para calculo de dias uteis.*/
Static Function VeriFerRCG(cFil, cMat, aEmp, nCont, x, aMeses, cTurno)

Local   dDataIni := aMeses[x][4] // recebe o primeiro dia do mùs corrente.
Local   cQuery // query
Local   aFerRCG := {}
...
atual ù um dia util para o funcionario.*/
While dDataIni <= aMeses[x][5]

      /*Se houve uma troca de turno pra esse funcionario,
      verifica se esse dia ù util nesse novo turno.*/
      If SPF->(MsSeek(cFil+cMat+dTos(dDataIni))) // se houve troca de turno,
considera o turno da tabela, SPF

            cQuery   := " SELECT RCG.RCG_DIAMES FROM "+U_RetSqlEmp(aEmp[nCont]
[1],"RCG")+" RCG "
      ...
      Else // se nùo houve troca de turno, considera o turno do cadastro de
funcionario.
...
semanas, dias nùo trabalhados do mùs -> Formato AAAAMMDD.
            (cTrbQry)->(dbSkip())
      EndDo
...
a regra ù a seguinte, se a data do inicio do afastamento ou do fim do
...
dias uteis for maior igual a 15 tras o registro, ou entùo se o funcionario
...
cQuery += "    AND (((SR8.R8_DATAINI BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
cQuery += "      OR SR8.R8_DATAFIM BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"') "
cQuery += "    AND SR8.R8_TIPO <> 'P') "
cQuery += "      OR ((SR8.R8_DATAINI BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
cQuery += "      OR SR8.R8_DATAFIM BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"') "
cQuery += "    AND (SR8.R8_TIPO = 'P' AND SR8.R8_DURACAO >= 16)) "
cQuery += "      OR (SR8.R8_DATAINI <= '"+dTos(aMeses[x][4])+"' AND SR8.R8_DATAFIM
>= '"+dTos(aMeses[x][5])+"' )) "
...
aFerRCG[y] -> Grava feriados, finais de semanas, dias nùo trabalhados no mùs dos
funcionarios -> Formato AAAAMMDD.

aAfast[y][1]   ->   Tipo de Afastamento -> 'P'='Atestado mùdico'
aAfast[y][2]   ->   Dia inicial afastamento -> Formato -> 'AAAAMMDD'
aAfast[y][3]   ->   Dia final do afastamento -> Formato -> 'AAAAMMDD'
aAfast[y][4]   ->   Nùmero de dias afastados tipo numerico
*/

Local dDataIni := aMeses[x][4] // recebe o primeiro dia do mùs corrente.
Local lFlag := .T. // Controla o teste de se o dia ù util em alguns dos testes.
Local nDiasUteis := 0 // quantidade de dias uteis desse funcionario nesse mùs.
Local cDiasUteis := "" // data dos dias uteis.
Local aDiasUteis := {} // Vetor com os dias a quantidade de dias uteis e a data dos
dias uteis.

SPF->(dbSetOrder(1)) // PF_FILIAL+PF_MAT+DTOS(PF_DATA)

/*Percorre todos os dias do mes do dia 1 ao ultimo dia
e verifica se aquele dia ù util para o funcionario.*/
While dDataIni <= aMeses[x][5]

     /*enquanto lFlag for .T. o dia ù util*/
     lFlag := .T. // a cada dia lFlag zera para teste novamente.

     /*se a data atual for maior ou igual a
     demissùo do funcionario sai do laùo.*/
     If (!Empty(dDemissao) .And. dDataIni >= dDemissao)
           Exit
     EndIf

     // testa pra ver se o dia ù de feriado, DSR, nùo trabalhado na tabela RCG.
     If aScan(aFerRCG, dTos(dDataIni)) <> 0
           lFlag := .F.
     EndIf

      If lFlag // se ainda nùo entrou em nenhuma das regras acima, testa na regra
de afastamento.

           // Percorre todos os afastamentos desse funcionario nesse mes.
           For y := 1 To Len(aAfast)

                  // se a data atual estiver entre o periodo de afastamento eo tipo
de afastamento for diferente de atestado medico.
                  If aAfast[y][1] <> 'P' .And. (dTos(dDataIni) >= aAfast[y]
[2] .And. dTos(dDataIni) <= aAfast[y][3])
                        lFlag := .F.
                  EndIf
                  // se a data atual estiver entre o periodo de afastamento eo tipo
de afastamento for igual a atestado medico
                  // conta a quantidade de dias uteis faltados de atestado nesse
mes depois de 15 dias.
                  If aAfast[y][1] == 'P' .And. ((dTos(dDataIni) >=
dTos(cTod(aAfast[y][2])+16)) .And. (dTos(dDataIni) <= aAfast[y][3]))
        ...
        If lFlag // se o dia for util soma.
              ...
              cDiasUteis += dTos(dDataIni)+'-' // esse dia ù util
        EndIf
...
nHoras := nHoras+(Int(nMinutos/60)) // 855/60 -> 14,25 -> 14 Horas + 160 horas ->
174 horas
If nMinutos >= 60 // Se minutos for maior ou igual a 60.
      nMinutos := Mod(nMinutos, 60) // Resto da divisùo dos minutos por 60 ->
855/60 -> 15 Minutos.
EndIf
cCHMesPrev := AllTrim(Str(nHoras))+"."+AllTrim(Str(nMinutos)) // Concatena Horas +
Minutos -> "174.15"
nCHMesPrev := Val(cCHMesPrev) // Transforma de Caracter pra numero -> 174.15

Return nCHMesPrev

/*Total de horas abonadas do funcionario posicionado no mùs*/
...
// SPH ->   ACUMULADOS DE APONTAMENTOS -> HISTORICO DOS APONTAMENTOS QUE JA
FECHARAM.
cQuery := " SELECT SUM(SOMAABONO) SOMAABONO FROM ( "
cQuery += " SELECT ISNULL(SUM(SPC.PC_QTABONO),0) SOMAABONO FROM
"+U_RetSqlEmp(aEmp[nCont][1],"SPC")+" SPC "
cQuery += " WHERE SPC.PC_FILIAL = '"+cFil+"' "
cQuery += "    AND SPC.PC_MAT = '"+cMat+"' "
cQuery += "    AND SPC.PC_DATA BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
cQuery += "    AND SPC.PC_ABONO <> '' "
cQuery += "    AND SPC.D_E_L_E_T_ = '' "
cQuery += " UNION ALL "
cQuery += " SELECT ISNULL(SUM(SPH.PH_QTABONO),0) SOMAABONO FROM
"+U_RetSqlEmp(aEmp[nCont][1],"SPH")+" SPH "
cQuery += " WHERE SPH.PH_FILIAL = '"+cFil+"' "
cQuery += "    AND SPH.PH_MAT = '"+cMat+"' "
cQuery += "    AND SPH.PH_DATA BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
...
nMinutos := (nSomaAbono - Int(nSomaAbono)) * 100 // ex: 16,9 - 16 -> 0.9 * 100 ->
90Minutos
nHoras := nHoras+(Int(nMinutos/60)) // ex: 16+((90/60) = 1,5) = 16 + 1 -> 17Horas
If nMinutos >= 60 // Se minutos for maior ou igual a 60.
      nMinutos := Mod(nMinutos, 60) // retorna o resto da divisùo, para retirar os
minutos corretos se os minutos for acima de 60 ex: 90m/60m resto -> 30min.
EndIf
cSomaAbono := AllTrim(Str(nHoras))+"."+AllTrim(Str(nMinutos)) // ex: "17"+"."+"30"
-> "17.30"
...
While !(cTrbQry)->(Eof())
        ...
        afastamento, para verificar se aquele dia do mes ù util
        para o funcioanrio.*/
        While dDataIni <= aMeses[x][5]
            /*se o dia que esta sendo lido estiver entre
            ...
            If (dTos(dDataIni) >= (cTrbQry)->INIAFAST .And. dTos(dDataIni) <=
(cTrbQry)->FIMAFAST) .And. dTos(dDataIni)$cDiasUteis
                  nDiasUtAt += 1
            EndIf
            dDataIni += 1
      EndDo
...
EndIf
cHoras := AllTrim(Str(nHoras))+"."+AllTrim(Str(nMinutos)) // "61"+"."+"15" ->
"61.15" -> horas minutos caracter
...
If nSomaMin >= 60 // Se minutos for maior ou igual a 60.
      nSomaMin := Mod(nSomaMin/60) // Resto da divisùo dos minutos por 60, para
encontrar os minutos.
EndIf
...
/*Percentual da hora nùo realizado em cima da hora prevista.*/
...
Local   nHoraNReal := 0 // Horas nùo realizadas.
Local   nMinNReal := 0 // Minutos das horas nùo realizadas.
...
Local cQuery // query
Local cVerbasHE := SuperGetMv("MV_VBHEPAG", .F., "('105',
'106','160','198','888','890','891')") // Verbas de Horas extras, busca no
parametro MV_VBHEPAG
Local cTrbQry := "QHE"
...
cQuery += "    AND SRC.RC_DATA BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
...
cQuery += "    AND SRD.RD_DATPGT BETWEEN '"+dTos(aMeses[x][4])+"' AND
'"+dTos(aMeses[x][5])+"' "
...
dbUseArea(.T., 'TOPCONN', TCGenQry(,,cQuery), cTrbQry, .F., .T.)
nHEPaga := fConvHr((cTrbQry)->HE, "H") // fConvHr converte Numero decimal para
horas, funùùo interna da TOTVS, encontrada no arquivo RHLIBHRS.PRX
(cTrbQry)->(dbCloseArea())
Return nHEPaga
...
Local cQuery // query
Local cVerbasFDs := SuperGetMv("MV_VBFDESC", .F., "('409', '412', '423',
'509')") // Verbas de Faltas/Descontos, busca no parametro MV_VBFDESC.
Local cTrbQry := "QFD"
...
dbUseArea(.T., 'TOPCONN', TCGenQry(,,cQuery), cTrbQry, .F., .T.)
nFTDesc := fConvHr((cTrbQry)->FD, "H") // fConvHr converte Numero decimal para
horas, funùùo interna da TOTVS, encontrada no arquivo RHLIBHRS.PRX
(cTrbQry)->(dbCloseArea())
...
While SPI->(!Eof()) .And. SPI->(PI_FILIAL+PI_MAT) == cFil+cMat .And. SPI->PI_DATA
<= aMeses[x][5]

      PosSP9(SPI->PI_PD,cFil,"P9_TIPOCOD") // nùo faco ideia do que faz ù funùùo
TOTVS.
...
/*Transforma tudo pra hora para impresùo no excel.*/
Static Function TransTudo(nCH, nCHMesPrev, nSomaAbono, nSomaAtest, nCHNaoReal,
nAbstsm, nHEPaga, nFTDesc, nPosNegTri)
...
Local nHCHNRea := 0 // Hora Nùo realizada.
Local nMCHNRea := 0 // Minutos nùo realizado.
...
cSomaAbono := AllTrim(Str(nHSAbono))+':'+AllTrim(StrZero(nMSAbono,2)) // Hora +
Minuto Abonado.

nHSAtest := Int(nSomaAtest) // Hora Atestado.
nMSAtest := (nSomaAtest - nHSAtest) * 100 // Minutos Atestado.
cSomaAtest := AllTrim(Str(nHSAtest))+':'+AllTrim(StrZero(nMSAtest,2)) // Hora +
Minuto atestado.

nHCHNRea := Int(nCHNaoReal) // Hora Nùo realizada.
nMCHNRea := (nCHNaoReal - nHCHNRea) * 100 // Minutos nùo realizado.
cCHNaoReal := AllTrim(Str(nHCHNRea))+':'+AllTrim(StrZero(nMCHNRea,2)) // Hora +
Minuto atestado.
...
cHEPaga := AllTrim(Str(nHHEPaga))+':'+AllTrim(Str(nMHEPaga)) // Hora + Minutos
extras

nHFTDesc := Int(nFTDesc) // Hora Falta e desconto
nMFTDesc := (nFTDesc-nHFTDesc) * 100 // Minuto falta e desconto
cFTDesc := AllTrim(Str(nHFTDesc))+':'+AllTrim(Str(nMFTDesc)) // Hora + Minutos
faltas e descontos
...
aTransTudo := {cCH, cCHMesPrev, cSomaAbono, cSomaAtest, cCHNaoReal, cAbstsm,
cHEPaga, cFTDesc, cPosTri, cNegTri} // Vetor com as horas transformadas.
